...
IOPS <- function(CountryCode ,tradeData , ComplexMethod = "eigenvalues", iterCompl = 20, GVCMapping = NULL, tradedigit = 6){
  ...
  Products <- unique(expdata$hs_product_code)
  Countries <- rownames(Mabs)
  ...
  if (is.null(GVCMapping) == TRUE) {
    ...
    merged_country_df <- subset(tradeData2018, location_code == chosenCountry)
    ...
    message("No Value Chain input provided, calculating default at all trade digit levels")
    ...
    GVCMapping <- GVCMapping[order(GVCMapping$tierNumber, GVCMapping$GVCactivityNumber, GVCMapping$HScode), ]

    numTiers <- length(unique(GVCMapping$tierNumber))         
    numActs <- length(unique(GVCMapping$GVCactivityNumber))   
    numProds <- length(unique(GVCMapping$HScode))             
